...
void huffman::createTree() {
    ...
    cout << "\nThe Characters With Their Frequency is: \n";
    while (tempPQ.size() != 1)
    {
        left = tempPQ.top();
        tempPQ.pop();
        if((left->data >= 'a' && left->data <= 'z') || (left->data >= 'A' && left->data <= 'Z')){
        ...
        right = tempPQ.top();
        tempPQ.pop();
        if((right->data >= 'a' && right->data <= 'z') || (right->data >= 'A' && left->data <= 'Z')){
...
void huffman::saveEncodedFile() {
    ...
    priority_queue <Node*, vector<Node*>, Compare> tempPQ(minHeap); //minheap->huffman tree
    while (!tempPQ.empty()) {
    ...
    ifstream input_file("inputfile.txt");  // open file for reading
    if (!input_file) {  // check if file was opened successfully
    ...
    cout << "\n\nThe Compressed File Now Has: ";
    ifstream input_file1("compressedfile.huf");  // open file for reading
    if (!input_file1) {  // check if file was opened successfully
        cerr << "Failed to open file\n";
    }

    std::string line1;
    while (std::getline(input_file1, line1)) {  // read file line by line
        cout << line1 << '\n';  // print each line to console
    }
...
void huffman::saveDecodedFile() {
    ...
    cout << "\n\nThe DeCompressed File Now Has: ";
    ifstream input_file("outputfile.txt");  // open file for reading
    if (!input_file) {  // check if file was opened successfully
        cerr << "Failed to open file\n";
    }
